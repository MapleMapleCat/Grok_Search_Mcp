...
function renderSecretModal(modal) {
  ...
  const body = `
    ...
    <div class="warning-callout">${renderIcon("warning")}<span>${modal.secretType === "invite" ? "请安全发送给目标用户。关闭窗口后，管理员仍可在邀请码列表中按需复制。" : "请立即复制并安全保存。关闭窗口后，API 密钥明文将无法再次获取。"}</span></div>
  `;
...
function renderUserUsageLogsModal(modal) {
  ...
  const footer = renderModalPagination({
    ...
    leadingContent: `<button class="button button-secondary" type="button" data-action="view-user-usage-summary" ${modal.loadingRecords ? "disabled" : ""}>返回用量摘要</button>`,
    pageAction: "change-user-usage-page",
    pageSizeAction: "change-user-usage-page-size"
  });
...
function renderTierModal(modal, isEdit) {
  const tier = modal.data || { name: "", rpm: 0, success_limit: 0, is_default: false };
  ...
  const body = `
    <form class="stack-form" id="tier-form" data-form="${formName}" ${isEdit ? `data-id="${escapeHTML(tier.id)}"` : ""}>
      ...
      <div class="form-grid">
        ...
        <label class="field-group"><span class="field-label"><span>月度成功调用额度</span><span class="field-hint">0 = 不限</span></span><input class="text-input" name="success_limit" type="number" min="0" step="1" value="${escapeHTML(tier.success_limit)}" required></label>
      </div>
  ...
  return renderModalFrame({ title: isEdit ? "编辑配额方案" : "创建配额方案", description: "方案控制用户限额；默认方案决定新用户的初始配额。", body, footer });
}

function renderCreateInviteModal(modal) {
  const body = `<form class="stack-form" id="create-invite-form" data-form="create-invite"><label class="field-group"><span class="field-label">最多注册人数</span><input class="text-input" name="registration_limit" type="number" min="1" step="1" value="1" required autofocus></label><div class="warning-callout">${renderIcon("warning")}<span>完整邀请码会加密保存，管理员之后仍可在列表中按需复制。邀请码可重复使用，直到达到注册人数上限或被管理员停用。</span></div>${modal.error ? `<div class="inline-alert">${renderIcon("alert")}<span>${escapeHTML(modal.error)}</span></div>` : ""}</form>`;
